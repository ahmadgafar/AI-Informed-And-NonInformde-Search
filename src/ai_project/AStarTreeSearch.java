...
public class AStarTreeSearch extends AbstractTreeSearch {

    boolean heuristic = true;

    public AStarTreeSearch(boolean heuristic) {
    ...
    public Collection<Node> expand(Node node, Problem problem) {
        ...
        for (Object action : actions) {
            ...
            if (!(State.repeatCheck(nextState, RepeatedState))) { // add the node with State next, parent node, the depth of parent node+1, 
                //the cost of the going to this node from root.
                if (heuristic) {
                    nodes.add(new Node(next, node, action, node.getDepth() + 1, problem.getStepCost(node, action, next) + hueristic1(next)));
                } else {
                    nodes.add(new Node(next, node, action, node.getDepth() + 1, problem.getStepCost(node, action, next) + hueristic2(next)));
                }
            }
        }
        return nodes;
